...
fn des_crypt(key: &[u8], clear: &[u8]) -> Vec<u8> {
    ...
    let cipher: Des = Des::new_from_slice(&exp_key).unwrap();
...
fn brute_twobytes(ntresponse: &[u8], challenge: &[u8]) -> Result<[u8; 2], ()> {
    let no_threads = 4;
    ...
    for j in 0..no_threads {
        ...
        thread::spawn(move || {
            ...
            for i in start..=end {
                ...
                if i % 100 == 0 && tx_thread.send(None).is_err() {
                    break;
                };
            }
        });
    ...
    while let Some(hashes) = rows.next().unwrap() {
        ...
        if let Ok(()) = check_hash(ntresponse, challenge, &chunk1, 0) {
            ...
            if let Ok(()) = check_hash(ntresponse, challenge, &chunk2, 8) {
                println!("[+] Full hash: {}{}{}", &hsh0, &hsh1, hex::encode(twobytes));
                found = true;
                break;
            }
        }
